...
#-------------------------------------------------------------------------------
# Name:        TXvios.py
# Purpose:     autobot for TX vios spreadsheets
#
# Author:      mthornton
#
# Created:     2017 JUN 09
...
def excelDataFill(csvRecord):
    #       0          1     2     3    4   5      6     7
    # responseType, plate, name, addr, '', city, state, zip,
        return [ '', '',
            csvRecord[3],
            csvRecord[5],
            csvRecord[6],
        ...
        for cell in lpList:
            ...
            if excelBool:
                ...
                for singleList in recordList:
                    txVioRecord = excelDataFill(singleList)
                    recordArray.append(txVioRecord)
...
def excelHook():
    ...
    # get LP from spreadsheet
    rawPlatesCol = [str( xlwings.Range((i,7)).value ) for i in indexList]
    plates = []
    # skip first 9 chars
    [plates.append(plate[9:]) for plate  in rawPlatesCol if plate != 'None' and plate != ""]
    l = len(plates)
    print l, plates
