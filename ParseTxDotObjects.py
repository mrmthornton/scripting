...
# the object has fields for  plate, format type,
# owner   -> name, owner2 name, address, address2, city, state, zip
...
class TxDotPlateInfo:
    def __init__(self, licencePlate, txdotString):
        self.rawData = txdotString
        self.responseType = ''
        self.plate = licencePlate
        ...
        parsedType, foundPlate,
        ownerName,   ownerName2,  ownerAddress,   ownerAddress2,   ownerCity,   ownerState,   ownerZip,
        renewalName,             renewalAddress, renewalAddress2, renewalCity, renewalState, renewalZip,
        issued, registered, assigned = parseRecord(self.plate, self.rawData)


        assert plate == foundPlate
        # decide on owner v renewal
        # decide on which date
